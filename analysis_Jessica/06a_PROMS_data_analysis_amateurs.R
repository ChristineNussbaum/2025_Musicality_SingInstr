##########################################################################
## File: 06a_PROMS_data_analysis_amateurs.R
...
P2 <- subset(P2, Code!="AEB56L") # instrumentalist

# rename P2 back to P to make script easier to read
P <- P2
rm(P2)
...
melody <- t.test(data = B[B$Test == "melody",], dprime ~ Group, paired = FALSE)
d_melody <- t_to_d(melody$statistic, melody$parameter, paired = FALSE)
pitch <- t.test(data = B[B$Test == "pitch",], dprime ~ Group, paired = FALSE)
d_pitch <- t_to_d(pitch$statistic, pitch$parameter, paired = FALSE)
timbre <- t.test(data = B[B$Test == "timbre",], dprime ~ Group, paired = FALSE)
d_timbre <- t_to_d(timbre$statistic, timbre$parameter, paired = FALSE)
rhythm <- t.test(data = B[B$Test == "rhythm",], dprime ~ Group, paired = FALSE)
d_rhythm <- t_to_d(rhythm$statistic, rhythm$parameter, paired = FALSE)



# run a simple t-test on mean Resp Accuracy
melodyII <- t.test(data = B[B$Test == "melody",], Resp ~ Group, paired = FALSE)
pitchII <- t.test(data = B[B$Test == "pitch",], Resp ~ Group, paired = FALSE)
timbreII <- t.test(data = B[B$Test == "timbre",], Resp ~ Group, paired = FALSE)
rhythmII <- t.test(data = B[B$Test == "rhythm",], Resp ~ Group, paired = FALSE)


# run a simple t-test on the criterion
melodyIII <- t.test(data = B[B$Test == "melody",], crit ~ Group, paired = FALSE)
pitchIII <- t.test(data = B[B$Test == "pitch",], crit ~ Group, paired = FALSE)
timbreIII <- t.test(data = B[B$Test == "timbre",], crit ~ Group, paired = FALSE)
rhythmIII <- t.test(data = B[B$Test == "rhythm",], crit ~ Group, paired = FALSE)


# run a simple t-test on the number of "unsure"-responses
melodyIV <- t.test(data = B[B$Test == "melody",], Unsure ~ Group, paired = FALSE)
pitchIV <- t.test(data = B[B$Test == "pitch",], Unsure ~ Group, paired = FALSE)
timbreIV <- t.test(data = B[B$Test == "timbre",], Unsure ~ Group, paired = FALSE)
rhythmIV <- t.test(data = B[B$Test == "rhythm",], Unsure ~ Group, paired = FALSE)


# run a simple t-test on the number of "DontKnow"-responses
melodyV <- t.test(data = B[B$Test == "melody",], DontKnow ~ Group, paired = FALSE)
pitchV <- t.test(data = B[B$Test == "pitch",], DontKnow ~ Group, paired = FALSE)
timbreV <- t.test(data = B[B$Test == "timbre",], DontKnow ~ Group, paired = FALSE)
rhythmV <- t.test(data = B[B$Test == "rhythm",], DontKnow ~ Group, paired = FALSE)


# run a simple t-tests on confidence (this is the measure used for the main manuscript)
melodyVI <- t.test(data = B[B$Test == "melody",], Confidence ~ Group, paired = FALSE)
d_melodyVI <- t_to_d(melodyVI$statistic, melodyVI$parameter, paired = FALSE)
pitchVI <- t.test(data = B[B$Test == "pitch",], Confidence ~ Group, paired = FALSE)
d_pitchVI <- t_to_d(pitchVI$statistic, pitchVI$parameter, paired = FALSE)
timbreVI <- t.test(data = B[B$Test == "timbre",], Confidence ~ Group, paired = FALSE)
d_timbreVI <- t_to_d(timbreVI$statistic, timbreVI$parameter, paired = FALSE)
rhythmVI <- t.test(data = B[B$Test == "rhythm",], Confidence ~ Group, paired = FALSE)
