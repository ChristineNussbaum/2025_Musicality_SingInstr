##########################################################################
## File: 03a_sample_information_amateurs_JS.R
...
# load the data of the survey2 and questionnaires that were collected before and after the classification experiment
...
#-> this dataset entails the information of 45 singers and 44 instrumentalists
...
# TimeTotal                         total time to complete the survey22 + experiment (minutes)
...
# Age
Age <- mySummary(survey2, Age, Group)

range <- survey2 %>% group_by(Group) %>% summarise(range(Age))

#Age
age_diff <- t.test(data = survey2, Age ~ Group, paired =  FALSE) 
...
## other demographics
sex <- table(survey2$Group, survey2$LSex)
language <- table(survey2$Group, survey2$LMotherLanguage)
profession <- table(survey2$LProfession)
hearingImp <- table(survey2$Group, survey2$LHearingImp)
hearingImp2 <- table(survey2$Group, survey2$LHearingImp2)
hearingImp3 <- table(survey2$Group, survey2$LHearingImp3)
duration <- survey2 %>% filter(TimeTotal < 999) %>%  group_by(Group) %>% summarise(Dur = mean(TimeTotal))
...
# descriptive_data
AQ_summary <- survey2 %>% group_by(Group) %>% summarise(AQ_Tot = mean(AQ_Total),   # AQ Total
...
#AQ TTest
AQ_total <- t.test(data = survey2, AQ_Total ~ Group, paired =  FALSE) 
d_AQ_total <- t_to_d(AQ_total$statistic, AQ_total$parameter, paired = FALSE)
AQ_ATT <- t.test(data = survey2, AQ_AttentionToDetails ~ Group, paired =  FALSE) 
d_AQ_ATT <- t_to_d(AQ_ATT$statistic, AQ_ATT$parameter, paired = FALSE)
AQ_Social <- t.test(data = survey2, AQ_Social ~ Group, paired =  FALSE) 
d_AQ_Social <- t_to_d(AQ_Social$statistic, AQ_Social$parameter, paired = FALSE)

AQ_SocSkills <- t.test(data = survey2, AQ_SocialSkills ~ Group, paired =  FALSE) 
d_AQ_SocSkills <- t_to_d(AQ_SocSkills$statistic, AQ_SocSkills$parameter, paired = FALSE)
AQ_Com <- t.test(data = survey2, AQ_Communication ~ Group, paired =  FALSE) 
d_AQ_Com <- t_to_d(AQ_Com$statistic, AQ_Com$parameter, paired = FALSE)
AQ_Imag <- t.test(data = survey2, AQ_Imagination ~ Group, paired =  FALSE) 
d_AQ_Imag <- t_to_d(AQ_Imag$statistic, AQ_Imag$parameter, paired = FALSE)
AQ_AttSwitch <- t.test(data = survey2, AQ_AttentionSwitching ~ Group, paired =  FALSE) 
...
# descriptive_data
PANAS <- survey2 %>% group_by(Group) %>% summarise(Neg = mean(neg_Aff),
                                                  NegSD = sd(neg_Aff),
                                                  Pos = mean(pos_Aff),
                                                  PosSD = sd(pos_Aff))

#PANAS
Neg <- t.test(data = survey2, neg_Aff ~ Group, paired =  FALSE) 
d_Neg <- t_to_d(Neg$statistic, Neg$parameter, paired = FALSE)
Pos <- t.test(data = survey2, pos_Aff ~ Group, paired =  FALSE) 
...
#Einkommen (Income)
Einkommen <- chisq.test(survey2$Group, survey2$Einkommen)

#Schulabschluss (Education)
Schulabschluss <- chisq.test(survey2$Group, survey2$Schulabschluss)

#Ausbildung (Degree)
Ausbildung <- chisq.test(survey2$Group, survey2$Ausbildung)

capture.output(Einkommen, table(survey2$Einkommen, survey2$Group),  Schulabschluss, table(survey2$Schulabschluss,survey2$Group), Ausbildung, table(survey2$Ausbildung, survey2$Group), file="output/sample_description/SES.txt")
...
# descriptive_data
MSI_summary <- survey2 %>% group_by(Group) %>% summarise(ME = mean(ME_mean), 
                                                       ME_SD = sd(ME_mean), 
...
#GOLD-MSI
ME <- t.test(data = survey2, ME_mean ~ Group, paired =  FALSE) 
d_ME <- t_to_d(ME$statistic, ME$parameter, paired = FALSE)
Active <- t.test(data = survey2, Mean_active ~ Group, paired =  FALSE) 
d_Active <- t_to_d(Active$statistic, Active$parameter, paired = FALSE)
Education <- t.test(data = survey2, Mean_education ~ Group, paired =  FALSE) 
d_Education <- t_to_d(Education$statistic, Education$parameter, paired = FALSE)
Emotion <- t.test(data = survey2, Mean_emotion ~ Group, paired =  FALSE) 
d_Emotion <- t_to_d(Emotion$statistic, Emotion$parameter, paired = FALSE)
Singing <- t.test(data = survey2, Mean_singing ~ Group, paired =  FALSE) 
d_Singing <- t_to_d(Singing$statistic, Singing$parameter, paired = FALSE)
Perception <- t.test(data = survey2, Mean_perception ~ Group, paired =  FALSE) 
...
# descriptive_data
OCEAN <- survey2 %>% group_by(Group) %>% summarise(o = round(mean(O), 2),
                                                  O_SD = round(sd(O), 2),
                                                  c = round(mean(C), 2),
                                                  C_SD = round(sd(C), 2),
                                                  e = round(mean(E), 2),
...
#OCEAN
O = t.test(data = survey2, O ~ Group, paired =  FALSE) 
d_O <- t_to_d(O$statistic, O$parameter, paired = FALSE)
C = t.test(data = survey2, C ~ Group, paired =  FALSE) 
d_C <- t_to_d(C$statistic, C$parameter, paired = FALSE)
E = t.test(data = survey2, E ~ Group, paired =  FALSE)
d_E <- t_to_d(E$statistic, E$parameter, paired = FALSE)
A = t.test(data = survey2, A ~ Group, paired =  FALSE) 
d_A <- t_to_d(A$statistic, A$parameter, paired = FALSE)
N = t.test(data = survey2, N ~ Group, paired =  FALSE) 
...
## singers only
singers <- survey2 %>% filter(Group =="1")
...
instrumentalists <- survey2 %>% filter(Group =="2")
