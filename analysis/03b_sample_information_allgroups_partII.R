##########################################################################
## File: 03b_sample_information_allgroups_partII.R
...
#------------------------------------------------------------------------------------------------------------------#
#                                   Combine data sets (amateurs + musicians/non-musicians)                                           #
#------------------------------------------------------------------------------------------------------------------#
...
MA_Age <- D %>% filter(Group == "M" | Group == "A") %>% t.test(data = ., Age ~ Group, paired=FALSE)
d_MA_Age <- t_to_d(MA_Age$statistic, MA_Age$parameter, paired = FALSE)
MC_Age <- D %>% filter(Group == "M" | Group == "C") %>% t.test(data = ., Age ~ Group, paired=FALSE)
d_MC_Age <- t_to_d(MC_Age$statistic, MC_Age$parameter, paired = FALSE)
AC_Age <- D %>% filter(Group == "A" | Group == "C") %>% t.test(data = ., Age ~ Group, paired=FALSE)
...
MA_neg <- D %>% filter(Group == "M" | Group == "A") %>% t.test(data = ., neg_Aff ~ Group, paired=FALSE)
d_MA_neg <- t_to_d(MA_neg$statistic, MA_neg$parameter, paired = FALSE)
MC_neg <- D %>% filter(Group == "M" | Group == "C") %>% t.test(data = ., neg_Aff ~ Group, paired=FALSE)
d_MC_neg <- t_to_d(MC_neg$statistic, MC_neg$parameter, paired = FALSE)
AC_neg <- D %>% filter(Group == "A" | Group == "C") %>% t.test(data = ., neg_Aff ~ Group, paired=FALSE)
...
MA_pos <- D %>% filter(Group == "M" | Group == "A") %>% t.test(data = ., pos_Aff ~ Group, paired=FALSE)
d_MA_pos <- t_to_d(MA_pos$statistic, MA_pos$parameter, paired = FALSE)
MC_pos <- D %>% filter(Group == "M" | Group == "C") %>% t.test(data = ., pos_Aff ~ Group, paired=FALSE)
d_MC_pos <- t_to_d(MC_pos$statistic, MC_pos$parameter, paired = FALSE)
AC_pos <- D %>% filter(Group == "A" | Group == "C") %>% t.test(data = ., pos_Aff ~ Group, paired=FALSE)
...
# perform pairwise t-tests, Bonferroni-corrected a-level: .05/3 = .0167
MA_AQTot <- D %>% filter(Group == "M" | Group == "A") %>% t.test(data = ., AQ_Total ~ Group, paired=FALSE)
d_MA_AQTot <- t_to_d(MA_AQTot$statistic, MA_AQTot$parameter, paired = FALSE)
MC_AQTot <- D %>% filter(Group == "M" | Group == "C") %>% t.test(data = ., AQ_Total ~ Group, paired=FALSE)
d_MC_AQTot <- t_to_d(MC_AQTot$statistic, MC_AQTot$parameter, paired = FALSE)
AC_AQTot <- D %>% filter(Group == "A" | Group == "C") %>% t.test(data = ., AQ_Total ~ Group, paired=FALSE)
d_AC_AQTot <- t_to_d(AC_AQTot$statistic, AC_AQTot$parameter, paired = FALSE)

#######################################################
#expected output (reduced)
# F-statistic: 2.904 on 2 and 163 DF,  p-value: 0.05765
#######################################################
